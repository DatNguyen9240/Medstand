...
(function () {
    ...
    function _resolveRowFields(r) {
        ...
        // Pattern detect: chỉ dng regex trn TN KEY, khng cần biết tn cụ thể
    ...
    function _menuShow(query) {
        ...
        var groups = {
            ...
            '🚀 Module nng cao (Advanced)': []
        };

        // Cc từ kha thuộc file 'Module common'
        var commonKeys = ['congno', 'danhmuc', 'tonkho', 'doanhso', 'donhang', 'hoadon', 'themkhachhang'];

        filtered.forEach(function (a) {
            // Nối cả ApiCode v DisplayName, bỏ hết dấu gạch dưới, khoảng trắng, k tự đặc biệt để so snh
            ...
            if (isCommon) {
                ...
                groups['🚀 Module nng cao (Advanced)'].push(a);
            }
        });
    ...
    function _showInlineValues(fieldCode, keyword, forceShowAll) {
        ...
        // Tm config của tham số hiện tại (v dụ: @ObjectID)
        ...
        // Nếu khng c DataSource, thng bo cho người dng biết để nhập tay v Enter
        if (!field || (!field.DataSourceType && !field.OptionsJson)) {
            ...
            var html = '<div class="ae-menu-item ae-val-item" style="color:var(--color-primary); font-style:italic;" '
                 + 'data-code="' + _esc(keyword) + '" data-name="Tm: \'' + _esc(keyword) + '\'">'
        ...
        // ── Fallback: dng catalogDsMap theo @Type đ chọn ────────────────
        // p dụng cả khi keyword rỗng (hiện ton bộ) hoặc khi c keyword (lọc)
        // Trigger khi field khng c dsVal ring HOẶC keyword rỗng (SP cần keyword)
        if (dsType === 'APICODE' && _pillParams && _catalogDsMap) {
            ...
            if (catalogFallbackDs && !dsVal) {
                ...
                // Hm render sau khi c rows (dng lại cho cả cache v API)
                function renderFallbackRows(allRows) {
                    ...
                    if (!rows.length) { 
                        ...
                        // Lun hiển thị thm ty chọn tm kiếm ton server ở cuối danh sch (nếu c g chữ)
                        if (keyword) {
                            html += '<div class="ae-menu-item ae-val-item" style="color:var(--color-primary); border-top:1px solid var(--color-border); margin-top:4px; padding-top:8px;" '
                                  + 'data-code="' + _esc(keyword) + '" data-name="Tm: \'' + _esc(keyword) + '\'">'
                                  + '<span class="ae-val-name">🔍 Tm tất cả sản phẩm chứa "' + _esc(keyword) + '"...</span>'
                                  + '</div>';
                        }
                    }
                    _menuEl.innerHTML = html;
        ...
        _loadDataSource(dsType, dsVal, keyword, function (rows) {
            ...
            displayRows.forEach(function (r) {
                ...
                // Lưu DataSourceValue vo _catalogDsMap để field sau dng
    ...
    function _menuPick() {
        ...
        if (idx < 0) {
            ...
            if (lastItem && String(lastItem.getAttribute('data-name')).indexOf('Tm:') === -1) {
    ...
    function _openPanel(config, execType, dispName) {
        ...
        if (execType === 'CART') {
            ...
            html += '<div class="ae-panel-empty">Khng c tham số  nhấn Gửi để thực hiện.</div>';
        } else {
            html += '<div class="ae-panel-fields">' + fields.map(_buildField).join('') + '</div>';
        }

        // Thm nt Gửi ngay trn panel
    ...
    function _buildField(f, _idx, allFields) {
        ...
        if (ctrl === 'combobox' && dsType) {
            input = '<div class="ae-combo" data-ds-type="' + _esc(dsType) + '" data-ds-val="' + _esc(dsVal) + '">'
                + '<input type="text" id="' + id + '_txt" class="ae-ctrl ae-combo-txt" placeholder="' + _esc(ph || 'G để tm...') + '" autocomplete="off"' + (reqd ? ' required' : '') + ' data-field="' + id + '">'
    ...
    function _addDataGridRow(gridId) {
        ...
        var html = '<div class="ae-dg-row" id="' + rowId + '">'
            ...
            + '<input type="text" class="ae-ctrl ae-combo-txt" placeholder="Tm hng..." autocomplete="off">'
        ...
        // Init DataSource cho ring dng ny
    ...
    function _renderComboSug(sug, txt, hid, rows, showAll) {
        ...
        if (!rows || !rows.length) {
            sug.innerHTML = '<div class="ae-sug-empty">Khng tm thấy kết quả</div>';
            sug.style.display = 'block';
            return;
        }
        var limit = showAll ? rows.length : 12;
        var html = '<div class="ae-sug-header"><span class="ae-sug-col-id">M / ID</span><span class="ae-sug-col-name">Thng tin chi tiết</span></div>';
    ...
    function _collectParams() {
        ...
        if (_activeApi.execType === 'CART') {
            if (!_cartItems.length) { alert('Vui lng thm t nhất 1 sản phẩm.'); return null; }
        ...
        // Cứu cnh: Đảm bảo tham số Ngy lun được gn tự động nếu người dng chỉ nhập keyword hoặc thiếu config
    ...
    function _executeApi(apiCode, params, dispName, execType, config) {
        ...
        var ps = Object.keys(params)
            ...
            .map(function (k) {
                // Tm label của field để hiển thị cho thn thiện
            ...
            username: _user() 
                ...
                if (arrData && arrData.length > 0) {
                    ...
                    if (dataRows.length === 0) {
                        _cbMsg && _cbMsg('ai', 'Dạ, em khng tm thấy dữ liệu no ph hợp với điều kiện vừa lọc ạ. 🙇‍♀️');
                        return;
                    }
                    ...
                    if (_cbRender && _cbHtml) {
                        ...
                        var html = _cbRender(dtToRender, r || ('🔍 Tm thấy ' + dtToRender.length + ' kết quả'), apiCode, {
                            uiTemplate: uiTpl,
            ...
            if (_activeApi) {
                ...
                if (lastAt !== -1) {
                    ...
                    // Nếu đ c phần '=', tức l đang g gi trị cho field
                        ...
                        // Cho php người dng g chuỗi di c dấu cch (v dụ: "băng c nhn"), 
                        // KHNG ngắt list gợi  giữa chừng chỉ v 1 dấu cch.

                        // Nếu l catalog token (@khachhang, @sanpham...) th dng catalog value dropdown
    ...
    window.ApiEngine = {
        ...
        // Dng khi người dng xa hash tag để giải phng API State
        ...
        showMenu: function (inputEl) {
            ...
            // Khng tự động thm k tự '@'  chỉ focus v mở menu
            _inputEl.focus();
