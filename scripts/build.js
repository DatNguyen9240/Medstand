...
`;
    ...
    cssTags.forEach((tag, index) => {
        if (index === 0) {
            prodHtmlContent = prodHtmlContent.replace(tag, '<link rel="stylesheet" href="src/css/dist/app.bundle.min.css?v=11.1">');
        } else {
    ...
    scriptTags.forEach((tag, index) => {
        if (index === 0) {
            prodHtmlContent = prodHtmlContent.replace(tag, '<script src="src/js/dist/app.bundle.min.js?v=11.1"></script>');
        } else {
            prodHtmlContent = prodHtmlContent.replace(tag, '');
        }
    });
    ...
    prodHtmlContent = prodHtmlContent.replace(
        ...
        '<script src="src/js/dist/theme.min.js?v=11.1"></script>'
    );

    // Thay thế script chatbot module bằng bundle chatbot-core
    prodHtmlContent = prodHtmlContent.replace(
        /<script\s+type=["']module["']\s+src=["']chatbot-widget\/js\/main\.js["']><\/script>/gi,
        '<script src="chatbot-widget/js/chatbot-core.bundle.min.js?v=11.1"></script>'
    );
    ...
    for (const fileRelPath of authHtmlFiles) {
        ...
        if (fs.existsSync(filePath)) {
            ...
            content = content.replace(
                ...
                'src="../src/js/dist/theme.min.js?v=11.1"'
            );
