...
const relativePath = 'sql/RAG-002_OCR_Review_AI.sql';
...
async function main() {
  ...
  if (context.DatabaseName !== 'medtest') throw new Error(`RAG-002 chỉ deploy trên medtest; hiện tại ${context.DatabaseName}.`);
  ...
  try {
    ...
    const required = ['AI_RagDocumentContentTbl', 'AI_RagDocumentReviewLogTbl', 'AI_ApprovedRagContentVw'];
    ...
    `)).recordset;
    if (objects.length !== required.length) throw new Error(`Thiếu object RAG-002 sau deploy: ${objects.length}/${required.length}.`);
...
main().catch((error) => {
  console.error(JSON.stringify({ task: 'RAG-002-DEPLOY', status: 'ERROR', error: error.message }, null, 2));
  process.exitCode = 1;
});
