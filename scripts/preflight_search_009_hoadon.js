...
/* Preflight cho SEARCH-005 áp dụng vào API_HoaDon_AI: before/after trên khách
...
const SQL_FILE = 'sql/Module_Common_API_HoaDon_AI.sql';
...
async function callHoaDon(request, username, maKhachHang) {
  ...
  const result = await r.execute('dbo.API_HoaDon_AI');
...
async function main() {
  ...
  try {
    ...
    // Khách có hóa đơn thật (AR_InvoiceTbl) và tên duy nhất toàn hệ thống.
    const sample = await new sql.Request(transaction).query(`
      ...
      WHERE O.isCustomer = 1 AND COALESCE(O.isDisable,0) = 0
        ...
        AND EXISTS (SELECT 1 FROM dbo.AR_InvoiceTbl D WHERE D.ObjectID = O.ObjectID)
      ORDER BY O.ObjectID`);
    expect(sample.recordset.length === 1, 'Không tìm được khách mẫu có hóa đơn + tên duy nhất để test hồi quy.');
    const uniqueTarget = sample.recordset[0];
    evidence.UniqueTarget = uniqueTarget;

    // ── 1) Hành vi TRƯỚC khi sửa ─────────────────────────────────────────────
    const before = await callHoaDon(transaction, 'Admin', uniqueTarget.ObjectName);
    expect(before.length >= 1, `Bản CŨ phải trả hóa đơn cho '${uniqueTarget.ObjectName}'.`);
    ...
    const afterUnique = await callHoaDon(transaction, 'Admin', uniqueTarget.ObjectName);
    ...
    const afterByCode = await callHoaDon(transaction, 'Admin', uniqueTarget.ObjectID);
    ...
    const ambiguous = await callHoaDon(transaction, 'Admin', dupName);
    ...
    console.log(JSON.stringify({
      Task: 'SEARCH-005-HOADON-AMBIGUITY-GUARD',
      Status: 'PASS',
...
main().catch((error) => {
  console.error(JSON.stringify({ Task: 'SEARCH-005-HOADON-AMBIGUITY-GUARD', Status: 'FAIL', Error: error.message }, null, 2));
  process.exitCode = 1;
});
