...
/* Preflight cho SEARCH-005 áp dụng vào API_TichLuy_AI (SP cuối trong 8 SP cần
   sửa). before/after trên khách khớp-đúng-1 để chống hồi quy, cộng một ca khớp
   nhiều khách phải NEEDS_SELECTION. --preflight thì luôn ROLLBACK ở cuối. */
const fs = require('fs');
const path = require('path');
const sql = require('mssql');

const ROOT = path.resolve(__dirname, '..');
const SQL_FILE = 'sql/Module_04_API_TichLuy_AI.sql';
...
async function callTichLuy(request, username, maKhachHang) {
  const r = new sql.Request(request)
    .input('Username', sql.VarChar(50), username)
    .input('MaKhachHang', sql.NVarChar(100), maKhachHang || '');
  const result = await r.execute('dbo.API_TichLuy_AI');
...
async function main() {
  ...
  try {
    ...
    const beforeByName = await callTichLuy(transaction, username, uniqueTarget.ObjectName);
    const beforeByCode = await callTichLuy(transaction, username, uniqueTarget.ObjectID);
    ...
    const afterByName = await callTichLuy(transaction, username, uniqueTarget.ObjectName);
    const afterByCode = await callTichLuy(transaction, username, uniqueTarget.ObjectID);
    ...
    expect(!(afterByName[0] && Number(afterByName[0].MsgType) === 2), 'Regression: khớp-đúng-1 không được lẫn vào nhánh NEEDS_SELECTION.');

    // ── 4) Hành vi MỚI: tên khớp NHIỀU khách phải NEEDS_SELECTION ───────────
    const dupName = 'Tâm Đức';
    const ambiguous = await callTichLuy(transaction, 'Admin', dupName);
    ...
    console.log(JSON.stringify({
      Task: 'SEARCH-005-TICHLUY-AMBIGUITY-GUARD',
      Status: 'PASS',
...
main().catch((error) => {
  console.error(JSON.stringify({ Task: 'SEARCH-005-TICHLUY-AMBIGUITY-GUARD', Status: 'FAIL', Error: error.message }, null, 2));
  process.exitCode = 1;
});
