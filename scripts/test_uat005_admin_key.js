...
async function main() {
  ...
  let receivedVerifiedUser = '';
  let receivedRequestId = '';
  const backend = http.createServer((req, res) => {
    if (req.url === '/api/API_UserInfo') {
      res.writeHead(200, { 'Content-Type': 'application/json' });
      res.end(JSON.stringify({ UserName: 'verified-rag-admin', Disable: 0 }));
      return;
    }
    receivedAdminKey = String(req.headers['x-admin-key'] || '');
    receivedVerifiedUser = String(req.headers['x-verified-user'] || '');
    receivedRequestId = String(req.headers['x-request-id'] || '');
    res.writeHead(200, { 'Content-Type': 'application/json' });
  ...
  try {
    ...
    await runGateway('test-admin-key-only', backendPort, async (result) => {
      ...
      assert.strictEqual(receivedVerifiedUser, 'verified-rag-admin');
      assert.match(receivedRequestId, /^req-/);
    });
    console.log('UAT-005 admin key and verified RAG identity: PASS 4/4');
  } finally {
