  ...
  return new Promise((resolve, reject) => {
    ...
    const request = http.request({
      ...
      headers: {
        ...
        'x-verified-user': 'RAG001_LIVE_UAT',
        'x-request-id': `req-rag001-live-${Date.now()}`,
      },
    }, (response) => {
      const output = [];
...
async function main() {
  ...
  try {
    ...
    try {
      if (pool) {
        await pool.request().input('Marker', sql.NVarChar(500), marker).query(`
          DELETE L FROM dbo.AI_RagDocumentReviewLogTbl AS L
          JOIN dbo.AI_RagDocumentTbl AS D ON D.DocumentID=L.DocumentID
          WHERE D.SourceReference=@Marker;
          DELETE C FROM dbo.AI_RagDocumentContentTbl AS C
          JOIN dbo.AI_RagDocumentTbl AS D ON D.DocumentID=C.DocumentID
          WHERE D.SourceReference=@Marker;
          DELETE L FROM dbo.AI_RagDocumentLifecycleLogTbl AS L
          JOIN dbo.AI_RagDocumentTbl AS D ON D.DocumentID=L.DocumentID
          WHERE D.SourceReference=@Marker;
          DELETE FROM dbo.AI_RagDocumentTbl WHERE SourceReference=@Marker;
        `);
        await pool.close();
      }
    } finally {
