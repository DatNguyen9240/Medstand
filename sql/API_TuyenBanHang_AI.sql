...
BEGIN
    ...
    SELECT TOP (@TopN)
        ...
        KH.ObjectName AS ObjectName, -- Cần thiết cho frontend hiển thị tên
        KH.Phone AS [Phone], 
        ...
        M.Latitude AS [Latitude],   -- Lấy toạ độ Latitude
        M.Longitude AS [Longitude]  -- Lấy toạ độ Longitude
    FROM CF_ObjectTbl KH
    JOIN #Logic L ON KH.ObjectID = L.ObjectID
    LEFT JOIN dbo.CF_ObjectMapTbl M ON KH.ObjectID = M.ObjectID -- JOIN với bảng toạ độ
