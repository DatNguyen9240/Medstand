...
BEGIN
    ...
    -- ═══════════════════════════════════════════════════
    -- KHÔNG TRUYỀN ObjectID → Top sản phẩm bán chạy nhất
    -- ═══════════════════════════════════════════════════
    IF @ObjectID = ''
    BEGIN
        SELECT TOP (@TopN)
        ...
        CASE
            ...
            THEN DATEDIFF(DAY, 
                    (SELECT MIN(I2.DocumentDate) 
                     ...
                     WHERE I2.ObjectID = @ObjectID AND D2.ItemID = L.ItemID AND I2.DocumentDate >= DATEADD(MONTH,-6,GETDATE())), 
                    L.LanMuaCuoi) 
                 / (L.SoLanMua - 1)
    ...
    WHERE I.ObjectID = @ObjectID AND MONTH(I.DocumentDate) = MONTH(GETDATE()) AND YEAR(I.DocumentDate) = YEAR(GETDATE()) - 1
    GROUP BY D.ItemID
