...
BEGIN
   ...
   END
    ...
    -- chỉ tự resolve khi khớp chính xác hoặc chỉ có đúng 1 khách gần đúng; nhiều
    -- hơn 1 thì trả NEEDS_SELECTION (MsgType=2) kèm danh sách để tầng gọi bắt
    -- người dùng chọn, thay vì đoán bừa một khách rồi trả nhầm công nợ.
     ...
     BEGIN
         ...
         --    tình KHÔNG áp dụng lối tắt này cho tên trùng khớp tuyệt đối: tên
         --    khách là text tự do, "Tâm Đức" có thể vừa là tên đầy đủ của đúng
         --    một khách vừa là chuỗi con của hàng trăm khách khác — phải đi qua
         --    bước đếm ambiguity bên dưới để không bỏ sót các khách đó.
         SELECT TOP 1 @ResolvedID = ObjectID
         FROM dbo.CF_ObjectTbl
         WHERE ObjectID = @CleanSearch
