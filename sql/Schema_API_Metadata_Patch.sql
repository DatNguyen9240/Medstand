...
-- ═══════════════════════════════════════════════════════════════════════════
-- BƯỚC 7: CẬP NHẬT API_GetConfig — FOR JSON PATH
-- Trả 1 row với FieldsJSON + FiltersJSON thay vì 4 resultset riêng
-- Fix lỗi "Missing para or Object not support2-" từ backend chatbot-action
-- ═══════════════════════════════════════════════════════════════════════════
    ...
    SELECT
        ...
        (
            SELECT F.FieldCode, F.FieldName, F.DataType, F.ControlType,
                   F.IsRequired, F.IsSystemParam, F.DefaultValue,
                   F.Placeholder, F.MinValue, F.MaxValue, F.OptionsJson,
...
END
GO
PRINT N'✅ API_GetConfig cập nhật xong (FOR JSON PATH)';
GO

/* ── TEST ──────────────────────────────────────────────────────────
EXEC API_GetConfig '@goi_y_don_hang';   -- Phải thấy cột FiltersJSON có data
EXEC API_GetConfig '@tra_cuu_san_pham'; -- FiltersJSON: [{FieldCode:"@SearchKey",...}]
EXEC API_GetConfig '@xem_don_hang';     -- FiltersJSON: 4 fields date/combobox/select
EXEC API_ListActive;
─────────────────────────────────────────────────────────────────── */
