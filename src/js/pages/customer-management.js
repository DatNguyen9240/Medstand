      ...
      function loadPage(page) {
        ...
        Http.get(API_CONFIG.ENDPOINTS.FILTER.CUSTOMERS, {
          ...
          .then(function (res) {
            ...
            $list.html(customers.length ? customers.map(renderCustomer).join('') : '<p style="text-align:center;color:var(--color-text-muted);padding:48px 0">Không có khách hàng</p>');
            window.scrollTo({ top: 0, behavior: 'smooth' });
          })
          .catch(function () {
            $('#skeleton').prop('hidden', true);
            $('#customer-list').prop('hidden', false).html('<p style="text-align:center;color:var(--color-text-muted);padding:48px 0">Không tải được dữ liệu</p>');
          });
      }
      ...
      .addList({
        ...
        loadFn: function (done) {
          Http.get(API_CONFIG.ENDPOINTS.FILTER.BRANCHES, { q: JSON.stringify({ BranchID: '', SearchText: '' }) })
            .then(function (res) {
              var records = (res.data || res).records || res.data || res || [];
              done(records.map(function (r) { return { value: r.BranchID || '', label: r.BranchName || r.BranchID || '' }; }));
            }).catch(function () { done([]); });
        }
      })
      .addList({
    ...
    if (user.BranchID) {
      Http.get(API_CONFIG.ENDPOINTS.FILTER.BRANCHES, { q: JSON.stringify({ BranchID: '', SearchText: '' }) })
        .then(function (res) {
          ...
          if (match) {
            _branchMatch = { value: match.BranchID, label: match.BranchName || match.BranchID };
            custForm.setListValue('branch', _branchMatch.value, _branchMatch.label);
    ...
    var _branchMatch = null; // cache kết quả auto-fill

    function openModal(editData) {
      custForm.reset();
      // Re-fill branch sau reset
      if (_branchMatch) {
        custForm.setListValue('branch', _branchMatch.value, _branchMatch.label);
      }
      if (editData) {
