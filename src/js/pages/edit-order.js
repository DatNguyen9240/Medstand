    ...
    function buildForm(summary) {
      ...
      if (user.BranchID) {
        ...
        // Force lock — addInput readonly không ngăn được click trong FormSelect
        setTimeout(function () {
          $('#fs-branch').off('click').css({ pointerEvents: 'none', opacity: '0.7' });
        }, 0);
      } else {
        orderForm.addList({
    ...
    function openProductPicker(rowId) {
      ...
      loadProducts(function (items) {
        $picker.attr('placeholder', 'Chọn sản phẩm').prop('disabled', false).css('opacity', '');
        ...
        $overlay.find('#pp-list li').on('click', function () {
          ...
          $picker.attr('data-value', val).attr('data-price', price).attr('data-name', name).val(name).addClass('has-value');
          $overlay.remove();
    ...
    function appendProductRow(prefill) {
      ...
      var actionBtns = isNew
        ...
        '<button type="button" class="btn-remove-row" onclick="removeProductRow(' + rowId + ')">&#x2715;</button>';
      var rowHtml = '<div class="responsive-grid add-product-row" id="row_' + rowId + '"' +
        ' data-auto-id="' + autoId + '"' +
        ' style="margin-bottom:8px;padding-bottom:16px;border-bottom:1px solid var(--color-border)">' +
        '<div class="form-group"><label class="form-label">Sản phẩm</label>' +
        '<input type="text" class="form-control" id="productPicker_' + rowId + '"' +
        ' data-value="' + (p.ItemID || '') + '"' +
        ' data-price="' + (p.UnitPrice || 0) + '"' +
        ' data-name="' + (p.ItemName || '') + '"' +
        ' value="' + (p.ItemName || '') + '"' +
        (p.ItemName ? ' class="form-control has-value"' : '') +
        ' readonly placeholder="Chọn sản phẩm" onclick="openProductPicker(' + rowId + ')" style="cursor:pointer"></div>' +
        '<div class="form-group"><label class="form-label">SL</label>' +
        '<input type="number" class="form-control" id="qty_' + rowId + '" value="' + (p.Quantity || 1) + '" min="1" oninput="calculateRowTotal(' + rowId + ')"></div>' +
        '<div class="form-group"><label class="form-label">Đơn giá</label>' +
        '<input type="number" class="form-control" id="price_' + rowId + '" value="' + (p.UnitPrice || '') + '" readonly></div>' +
        '<div class="form-group"><label class="form-label">CK(%)</label>' +
        '<input type="number" class="form-control" id="discount_' + rowId + '" value="' + (p.DiscountPercent || 0) + '" min="0" max="100" oninput="calculateRowTotal(' + rowId + ')"></div>' +
        ...
        '<div class="form-group" style="display:flex;flex-direction:column;align-items:center;justify-content:flex-end;gap:2px">' +
        actionBtns + '</div>' +
        '</div>';
