...
Write-Output "=========================================================="
Write-Output "MEDSTAND AI - THUC THI KIEM TRA DU LIEU 19 TINH NANG (CAP 2)"
Write-Output "Cap: QLBH016.MED (Tran Van Huong) - BACNINHA.MED (Nguyen Cong Duc)"
Write-Output "Khach hang mau: Quay Thuoc Thu Thuy (HYA107)"
...
$dsQL = Execute-Query "EXEC API_DoanhSo_AI @Username = 'QLBH016.MED', @LoaiBaoCao = 'NhanVien'"
$dsTDV = Execute-Query "EXEC API_DoanhSo_AI @Username = 'BACNINHA.MED', @LoaiBaoCao = 'NhanVien'"
if ($dsQL) { Write-Output "  * Quan ly QLBH016.MED: Xem thay $($dsQL.Rows.Count) nhan vien trong vung." }
if ($dsTDV) { 
    Write-Output "  * TDV BACNINHA.MED: Xem thay $($dsTDV.Rows.Count) nhan vien (chi chinh minh)."
    if ($dsTDV.Rows.Count -gt 0) {
        $val = $dsTDV.Rows[0][7]
        Write-Output "  * Doanh so Nguyen Cong Duc (TDV): $val VND."
    }
}
Write-Output ""

# STT 02: Bao mat - phan quyen
Write-Output "--- STT 02: Bao Mat Phan Quyen (Quan ly MB xem Mien Nam) ---"
$dsCross = Execute-Query "EXEC API_DoanhSo_AI @Username = 'QLBH016.MED', @ManagerID = 'QLMN2', @LoaiBaoCao = 'NhanVien'"
$isBlocked = $true
if ($dsCross) {
    foreach ($row in $dsCross.Rows) {
        if ($row[6] -ne 'MB') { $isBlocked = $false }
    }
    Write-Output "  * Ket qua loc cross-zone: $($dsCross.Rows.Count) dong tra ve. Tat ca deu thuoc chi nganh MB."
} else {
...
$dhQL = Execute-Query "EXEC API_DonHang_AI @Username = 'QLBH016.MED', @TopN = 5000"
$dhTDV = Execute-Query "EXEC API_DonHang_AI @Username = 'BACNINHA.MED', @TopN = 5000"
if ($dhQL) { Write-Output "  * Quan ly QLBH016.MED: Xem thay tong cong $($dhQL.Rows.Count) don hang cua vung." }
if ($dhTDV) { Write-Output "  * TDV BACNINHA.MED: Xem thay tong cong $($dhTDV.Rows.Count) don hang ca nhan." }
Write-Output ""
...
Write-Output "--- STT 05: Goi Y Dat Hang Tu Dong cho HYA107 ---"
$goiY = Execute-Query "EXEC API_GoiYDonHang_AI @Username = 'BACNINHA.MED', @MaKhachHang = 'HYA107', @TopN = 3"
...
Write-Output "--- STT 06: Goi Y Upsell/Ban Kem cho HYA107 ---"
$upsell = Execute-Query "EXEC API_UpsellGoiY_AI @Username = 'BACNINHA.MED', @MaKhachHang = 'HYA107', @TopN = 3"
...
$sp = Execute-Query "EXEC API_TraCuuSanPham_AI @Username = 'BACNINHA.MED', @timkiem = 'Antrinano', @TopN = 1"
if ($sp) {
...
$cnQL = Execute-Query "EXEC API_CongNoKhachHang_AI @Username = 'QLBH016.MED', @DenNgay = '2026-05-25'"
$cnTDV = Execute-Query "EXEC API_CongNoKhachHang_AI @Username = 'BACNINHA.MED', @DenNgay = '2026-05-25'"
if ($cnQL) { Write-Output "  * Quan ly QLBH016.MED: Xem thay $($cnQL.Rows.Count) khach hang no trong vung." }
if ($cnTDV) { 
    Write-Output "  * TDV BACNINHA.MED: Xem thay $($cnTDV.Rows.Count) khach hang no thuoc tuyen."
...
Write-Output "--- STT 09: Cong No Chi Tiet cho HYA107 ---"
$cnCT = Execute-Query "EXEC API_CongNoChiTiet_AI @Username = 'BACNINHA.MED', @MaKhachHang = 'HYA107', @DenNgay = '2026-05-25'"
if ($cnCT -and $cnCT.Rows.Count -gt 0) {
    ...
    Write-Output "  * Khach hang HYA107 hien khong co no chi tiet."
}
Write-Output ""

# STT 10: Tra cuu hoa don ban hang
Write-Output "--- STT 10: Tra Cuu Hoa Don Ban Hang ---"
$hdQL = Execute-Query "EXEC API_HoaDon_AI @Username = 'QLBH016.MED', @timkiem = 'HYA107'"
$hdTDV = Execute-Query "EXEC API_HoaDon_AI @Username = 'BACNINHA.MED', @timkiem = 'HYA107'"
if ($hdQL) { Write-Output "  * So hoa don cua HYA107 tim thay boi Quan ly: $($hdQL.Rows.Count) hoa don." }
if ($hdTDV) { Write-Output "  * So hoa don cua HYA107 tim thay boi TDV: $($hdTDV.Rows.Count) hoa don." }
Write-Output ""

# STT 11: Diem tich luy khach hang
Write-Output "--- STT 11: Diem Tich Luy Khach Hang cho HYA107 ---"
$tlQL = Execute-Query "EXEC API_TichLuy_AI @Username = 'QLBH016.MED', @MaKhachHang = 'HYA107'"
$tlTDV = Execute-Query "EXEC API_TichLuy_AI @Username = 'BACNINHA.MED', @MaKhachHang = 'HYA107'"
if ($tlTDV -and $tlTDV.Rows.Count -gt 0) {
    ...
    Write-Output "  * Khong tim thay diem tich luy cua khach hang HYA107."
}
Write-Output ""

# STT 12: Tuyen ban hang hang ngay
Write-Output "--- STT 12: Tuyen Ban Hang Hang Ngay ---"
$tuyenQL = Execute-Query "EXEC API_TuyenBanHang_AI @Username = 'QLBH016.MED', @TopN = 500"
$tuyenTDV = Execute-Query "EXEC API_TuyenBanHang_AI @Username = 'BACNINHA.MED', @TopN = 500"
...
$donThuoc = Execute-Query "EXEC API_GoiYDonThuoc_AI @Username = 'BACNINHA.MED', @timkiem = 'Amoxicillin, Vitamin C'"
if ($donThuoc) {
...
$km = Execute-Query "EXEC API_DeXuatKhuyenMai_AI @Username = 'BACNINHA.MED'"
if ($km) {
...
$tt = Execute-Query "EXEC API_SanPhamTrongTam_AI @Username = 'BACNINHA.MED'"
if ($tt) {
...
$dm = Execute-Query "EXEC API_DanhMuc_AI @Username = 'BACNINHA.MED', @Type = 'sanpham'"
if ($dm) {
...
Write-Output "--- STT 17: RFM-C Cham Diem Tin Nhiem (HYA107) ---"
$rfmTDV = Execute-Query "EXEC API_ChamDiemKH_AI @Username = 'BACNINHA.MED', @MaKhachHang = 'HYA107', @W_Recency = 30.0, @W_Frequency = 25.0, @W_Monetary = 35.0, @W_Consumption = 10.0"
if ($rfmTDV -and $rfmTDV.Rows.Count -gt 0) {
    $rfmTDV | Format-Table -AutoSize | Out-String | Write-Output
} else {
    Write-Output "  * Khong tim thay diem tin nhiem cho HYA107."
}
Write-Output ""

# STT 18: Kiem tra ton kho thuc te (A003)
Write-Output "--- STT 18: Kiem Tra Ton Kho Thuc Te (A003) ---"
$tk = Execute-Query "EXEC API_DanhSachTonKho_AI @Username = 'BACNINHA.MED', @ItemID = 'A003'"
if ($tk) {
...
$ks = Execute-Query "EXEC API_KiemTraKhaoSat_AI @Username = 'BACNINHA.MED', @Ngay = NULL"
if ($ks) {
