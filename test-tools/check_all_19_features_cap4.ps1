...
Write-Output "=========================================================="
Write-Output "MEDSTAND AI - THUC THI KIEM TRA DU LIEU 19 TINH NANG (CAP 4)"
Write-Output "Cap: QLBH010.MED (Nguyen Van Viet Anh) - DANANGA.MED (Le Thi Le)"
...
$dsQL = Execute-Query "EXEC API_DoanhSo_AI @Username = 'QLBH010.MED', @LoaiBaoCao = 'NhanVien'"
$dsTDV = Execute-Query "EXEC API_DoanhSo_AI @Username = 'DANANGA.MED', @LoaiBaoCao = 'NhanVien'"
if ($dsQL) { Write-Output "  * Quan ly QLBH010.MED: Xem thay $($dsQL.Rows.Count) nhan vien trong vung." }
if ($dsTDV) { 
    Write-Output "  * TDV DANANGA.MED: Xem thay $($dsTDV.Rows.Count) nhan vien (chi chinh minh)."
    if ($dsTDV.Rows.Count -gt 0) {
        $val = $dsTDV.Rows[0][7]
        Write-Output "  * Doanh so Le Thi Le (TDV): $val VND."
    }
}
Write-Output ""

# STT 02: Bao mat - phan quyen
Write-Output "--- STT 02: Bao Mat Phan Quyen (Quan ly MT xem Mien Nam) ---"
$dsCross = Execute-Query "EXEC API_DoanhSo_AI @Username = 'QLBH010.MED', @ManagerID = 'QLMN2', @LoaiBaoCao = 'NhanVien'"
$isBlocked = $true
if ($dsCross) {
...
$dhQL = Execute-Query "EXEC API_DonHang_AI @Username = 'QLBH010.MED', @TopN = 5000"
$dhTDV = Execute-Query "EXEC API_DonHang_AI @Username = 'DANANGA.MED', @TopN = 5000"
if ($dhQL) { Write-Output "  * Quan ly QLBH010.MED: Xem thay tong cong $($dhQL.Rows.Count) don hang cua vung." }
if ($dhTDV) { Write-Output "  * TDV DANANGA.MED: Xem thay tong cong $($dhTDV.Rows.Count) don hang ca nhan." }
Write-Output ""
...
$goiY = Execute-Query "EXEC API_GoiYDonHang_AI @Username = 'DANANGA.MED', @MaKhachHang = 'DNA014', @TopN = 3"
...
$upsell = Execute-Query "EXEC API_UpsellGoiY_AI @Username = 'DANANGA.MED', @MaKhachHang = 'DNA014', @TopN = 3"
...
$sp = Execute-Query "EXEC API_TraCuuSanPham_AI @Username = 'DANANGA.MED', @timkiem = 'Antrinano', @TopN = 1"
if ($sp) {
...
$cnQL = Execute-Query "EXEC API_CongNoKhachHang_AI @Username = 'QLBH010.MED', @DenNgay = '2026-05-25'"
$cnTDV = Execute-Query "EXEC API_CongNoKhachHang_AI @Username = 'DANANGA.MED', @DenNgay = '2026-05-25'"
if ($cnQL) { Write-Output "  * Quan ly QLBH010.MED: Xem thay $($cnQL.Rows.Count) khach hang no trong vung." }
if ($cnTDV) { 
    Write-Output "  * TDV DANANGA.MED: Xem thay $($cnTDV.Rows.Count) khach hang no thuoc tuyen."
...
$cnCT = Execute-Query "EXEC API_CongNoChiTiet_AI @Username = 'DANANGA.MED', @MaKhachHang = 'DNA014', @DenNgay = '2026-05-25'"
...
$hdQL = Execute-Query "EXEC API_HoaDon_AI @Username = 'QLBH010.MED', @timkiem = 'DNA014'"
$hdTDV = Execute-Query "EXEC API_HoaDon_AI @Username = 'DANANGA.MED', @timkiem = 'DNA014'"
...
$tlQL = Execute-Query "EXEC API_TichLuy_AI @Username = 'QLBH010.MED', @MaKhachHang = 'DNA014'"
$tlTDV = Execute-Query "EXEC API_TichLuy_AI @Username = 'DANANGA.MED', @MaKhachHang = 'DNA014'"
...
$tuyenQL = Execute-Query "EXEC API_TuyenBanHang_AI @Username = 'QLBH010.MED', @TopN = 500"
$tuyenTDV = Execute-Query "EXEC API_TuyenBanHang_AI @Username = 'DANANGA.MED', @TopN = 500"
...
$donThuoc = Execute-Query "EXEC API_GoiYDonThuoc_AI @Username = 'DANANGA.MED', @timkiem = 'Amoxicillin, Vitamin C'"
if ($donThuoc) {
...
$km = Execute-Query "EXEC API_DeXuatKhuyenMai_AI @Username = 'DANANGA.MED'"
if ($km) {
...
$tt = Execute-Query "EXEC API_SanPhamTrongTam_AI @Username = 'DANANGA.MED'"
if ($tt) {
...
$dm = Execute-Query "EXEC API_DanhMuc_AI @Username = 'DANANGA.MED', @Type = 'sanpham'"
if ($dm) {
...
$rfmTDV = Execute-Query "EXEC API_ChamDiemKH_AI @Username = 'DANANGA.MED', @MaKhachHang = 'DNA014', @W_Recency = 30.0, @W_Frequency = 25.0, @W_Monetary = 35.0, @W_Consumption = 10.0"
...
$tk = Execute-Query "EXEC API_DanhSachTonKho_AI @Username = 'DANANGA.MED', @ItemID = 'A003'"
if ($tk) {
...
$ks = Execute-Query "EXEC API_KiemTraKhaoSat_AI @Username = 'DANANGA.MED', @Ngay = NULL"
if ($ks) {
