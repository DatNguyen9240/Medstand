...
Write-Output "=========================================================="
Write-Output "MEDSTAND AI - THUC THI KIEM TRA DU LIEU 19 TINH NANG (CAP 5)"
Write-Output "Cap: QLMN2 (Tran Van Luan) - CanThoA (Nguyen Thi Thu Thao)"
...
$dsQL = Execute-Query "EXEC API_DoanhSo_AI @Username = 'QLMN2', @LoaiBaoCao = 'NhanVien'"
$dsTDV = Execute-Query "EXEC API_DoanhSo_AI @Username = 'CanThoA', @LoaiBaoCao = 'NhanVien'"
if ($dsQL) { Write-Output "  * Quan ly QLMN2: Xem thay $($dsQL.Rows.Count) nhan vien trong vung." }
if ($dsTDV) { 
    Write-Output "  * TDV CanThoA: Xem thay $($dsTDV.Rows.Count) nhan vien (chi chinh minh)."
    if ($dsTDV.Rows.Count -gt 0) {
        $val = $dsTDV.Rows[0][7]
        Write-Output "  * Doanh so Nguyen Thi Thu Thao (TDV): $val VND."
    }
}
Write-Output ""
...
$dsCross = Execute-Query "EXEC API_DoanhSo_AI @Username = 'QLMN2', @ManagerID = 'QLBH013.MED', @LoaiBaoCao = 'NhanVien'"
$isBlocked = $true
if ($dsCross) {
...
$dhQL = Execute-Query "EXEC API_DonHang_AI @Username = 'QLMN2', @TopN = 5000"
$dhTDV = Execute-Query "EXEC API_DonHang_AI @Username = 'CanThoA', @TopN = 5000"
if ($dhQL) { Write-Output "  * Quan ly QLMN2: Xem thay tong cong $($dhQL.Rows.Count) don hang cua vung." }
if ($dhTDV) { Write-Output "  * TDV CanThoA: Xem thay tong cong $($dhTDV.Rows.Count) don hang ca nhan." }
Write-Output ""
...
$goiY = Execute-Query "EXEC API_GoiYDonHang_AI @Username = 'CanThoA', @MaKhachHang = 'DNA014', @TopN = 3"
...
$upsell = Execute-Query "EXEC API_UpsellGoiY_AI @Username = 'CanThoA', @MaKhachHang = 'DNA014', @TopN = 3"
...
$sp = Execute-Query "EXEC API_TraCuuSanPham_AI @Username = 'CanThoA', @timkiem = 'Antrinano', @TopN = 1"
if ($sp) {
...
$cnQL = Execute-Query "EXEC API_CongNoKhachHang_AI @Username = 'QLMN2', @DenNgay = '2026-05-25'"
$cnTDV = Execute-Query "EXEC API_CongNoKhachHang_AI @Username = 'CanThoA', @DenNgay = '2026-05-25'"
if ($cnQL) { Write-Output "  * Quan ly QLMN2: Xem thay $($cnQL.Rows.Count) khach hang no trong vung." }
if ($cnTDV) { 
    Write-Output "  * TDV CanThoA: Xem thay $($cnTDV.Rows.Count) khach hang no thuoc tuyen."
...
$cnCT = Execute-Query "EXEC API_CongNoChiTiet_AI @Username = 'CanThoA', @MaKhachHang = 'DNA014', @DenNgay = '2026-05-25'"
...
$hdQL = Execute-Query "EXEC API_HoaDon_AI @Username = 'QLMN2', @timkiem = 'DNA014'"
$hdTDV = Execute-Query "EXEC API_HoaDon_AI @Username = 'CanThoA', @timkiem = 'DNA014'"
...
$tlQL = Execute-Query "EXEC API_TichLuy_AI @Username = 'QLMN2', @MaKhachHang = 'DNA014'"
$tlTDV = Execute-Query "EXEC API_TichLuy_AI @Username = 'CanThoA', @MaKhachHang = 'DNA014'"
...
$tuyenQL = Execute-Query "EXEC API_TuyenBanHang_AI @Username = 'QLMN2', @TopN = 500"
$tuyenTDV = Execute-Query "EXEC API_TuyenBanHang_AI @Username = 'CanThoA', @TopN = 500"
...
$donThuoc = Execute-Query "EXEC API_GoiYDonThuoc_AI @Username = 'CanThoA', @timkiem = 'Amoxicillin, Vitamin C'"
if ($donThuoc) {
...
$km = Execute-Query "EXEC API_DeXuatKhuyenMai_AI @Username = 'CanThoA'"
if ($km) {
...
$tt = Execute-Query "EXEC API_SanPhamTrongTam_AI @Username = 'CanThoA'"
if ($tt) {
...
$dm = Execute-Query "EXEC API_DanhMuc_AI @Username = 'CanThoA', @Type = 'sanpham'"
if ($dm) {
...
$rfmTDV = Execute-Query "EXEC API_ChamDiemKH_AI @Username = 'CanThoA', @MaKhachHang = 'DNA014', @W_Recency = 30.0, @W_Frequency = 25.0, @W_Monetary = 35.0, @W_Consumption = 10.0"
...
$tk = Execute-Query "EXEC API_DanhSachTonKho_AI @Username = 'CanThoA', @ItemID = 'A003'"
if ($tk) {
...
$ks = Execute-Query "EXEC API_KiemTraKhaoSat_AI @Username = 'CanThoA', @Ngay = NULL"
if ($ks) {
